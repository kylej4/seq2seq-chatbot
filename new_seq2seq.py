...
voc, pairs = loadPrepareData('/workspace/chatbot/data/train.src', '/workspace/chatbot/data/train.tgt')
#voc_val, pairs_val = loadPrepareData('/workspace/workspace/chatbot/data/valid.src', '/workspace/workspace/chatbot/data/valid.tgt')
#voc_test, pairs_test = loadPrepareData('/workspace/chatbot/data/test.src', '/workspace/chatbot/data/test.tgt')
...
class DecoderRNN(nn.Module):
    ...
    def forward(self, input_step, last_hidden, encoder_outputs):
        ...
        out_temp = nn.Linear(encoder_outputs.size(0),1).to(device)
        context = out_temp(encoder_outputs.transpose(0,2))
        #print("context1:",context.shape)

        context = context.transpose(0,2)
        #print("context2:",context.shape)
        #print("context:", context.shape, context)
        # 문백 벡터와 GRU 출력을 결합합니다
        rnn_output = rnn_output.squeeze(0)
        context = context.reshape(context.size(1),500)
        #print("context after reshape:", context.shape, context)
...
def train(input_variable, lengths, target_variable, mask, max_target_len, encoder, decoder, embedding,
    ...
    print("train encoder output:", encoder_outputs.shape)
...
class GreedySearchDecoder(nn.Module):
    ...
    def forward(self, input_seq, input_length, max_length):
        ...
        print("evaluate encoder output:", encoder_outputs.shape)
...
def evaluateInput(encoder, decoder, searcher, voc):
    ...
    hyp = []
    H_pair = []
    h = open('/workspace/new_start/hyp.txt', 'w')
    r = open('/workspace/new_start/ref.txt', 'w')
    for pair in pairs:
        # 입력 문장을 받아옵니다
        input_sentence = pair[0]
        reference = pair[1]
...
#                            '2-2_500'.format(encoder_n_layers, decoder_n_layers, hidden_size),
